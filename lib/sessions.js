...
async function cleanupIdleSessions(reason = 'idle-timeout') {
  ...
  for (const row of rows) {
    const lastSeenMs = parseStoredDateMs(row.last_seen_at);
    const expiresMs = parseStoredDateMs(row.expires_at);
...
async function touchSessionForToken(user, token, req) {
  ...
  const now = Date.now();
  const lastSeenMs = parseStoredDateMs(session.last_seen_at);
  const expiresMs = parseStoredDateMs(session.expires_at);
