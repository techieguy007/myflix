...
function loadConfig(configPath = process.env.MYFLIX_CONFIG || DEFAULT_CONFIG_PATH) {
  ...
  config.auth.idleMinutes = Number(process.env.MYFLIX_SESSION_IDLE_MINUTES || config.auth.idleMinutes || 120);
  config.auth.sessionCleanupIntervalMinutes = Number(
    process.env.MYFLIX_SESSION_CLEANUP_INTERVAL_MINUTES || config.auth.sessionCleanupIntervalMinutes || 5
  );
