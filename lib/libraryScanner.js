...
async function loadKnownScannerFiles() {
  const rows = await db.all(`
    SELECT id, video_path, original_path, file_size, file_mtime_ms
    FROM movies
  ...
  for (const row of rows) {
    if (row.video_path) {
      known.set(pathKey(row.video_path), row);
    }
    if (row.original_path
      && row.video_path
      && pathKey(row.original_path) !== pathKey(row.video_path)
      && fs.existsSync(row.video_path)) {
      known.set(pathKey(row.original_path), {
        ...row,
        replacedByConverted: true
      });
    }
  }
...
function isKnownUnchanged(file, existing) {
  if (!existing) return false;

  const knownSize = Number(existing.file_size);
  if (Number.isFinite(knownSize) && knownSize !== Number(file.stat.size)) {
    return false;
  }

  const knownMtime = Number(existing.file_mtime_ms);
  if (Number.isFinite(knownMtime) && knownMtime > 0) {
    return Math.abs(knownMtime - Number(file.stat.mtimeMs)) < 1000;
  }

  return Number.isFinite(knownSize);
}

async function prepareScanCandidates(files, root, config, counters, options = {}) {
  const minDurationSeconds = Number(config.media.minDurationMinutes || 15) * 60;
  const candidates = new Map();
  const skippedKnownPaths = [];
  const knownFiles = options.knownFiles || new Map();
  ...
  for (const file of files) {
    ...
    const knownFile = knownFiles.get(pathKey(file.fullPath));
    if (knownFile && knownFile.replacedByConverted) {
      counters.skippedKnown += 1;
      continue;
    }

    if (!forceRescan && isKnownUnchanged(file, knownFile)) {
      skippedKnownPaths.push(file.fullPath);
      counters.skippedKnown += 1;
      continue;
    }

    if (!forceRescan && knownFile) {
      counters.changed += 1;
    }
...
async function upsertMediaItem(item, root, config, counters) {
  ...
  const fileMtimeMs = Number.isFinite(finalStat.mtimeMs) ? finalStat.mtimeMs : null;
  ...
  if (existing) {
    await db.run(`
      UPDATE movies SET
        ...
        video_path = ?, file_size = ?, file_mtime_ms = ?, format = ?,
    ...
    await db.run(`
      INSERT INTO movies (
        ...
        video_path, file_size, file_mtime_ms, format, poster_url, imdb_id, imdb_rating,
...
async function removeStaleScannerRows(currentPaths) {
  const existing = await db.all('SELECT id, video_path FROM movies WHERE library_source = ?', ['scanner']);
  const current = new Set(currentPaths.map((item) => item.toLowerCase()));
  let removed = 0;
...
async function removeMissingMovieRows() {
  const existing = await db.all(`
    SELECT id, title, video_path, library_source
    FROM movies
  ...
  for (const row of existing) {
    if (!fs.existsSync(row.video_path)) {
      await deleteMovieRow(row);
      removed += 1;
      logger.warn('library.missing_file_row_removed', {
        movieId: row.id,
        title: row.title,
        videoPath: row.video_path,
        librarySource: row.library_source || null
      });
    }
  }
...
async function runLibraryScan(options = {}) {
  ...
  const forceRescan = Boolean(options.forceRescan || options.force);
  ...
  try {
    ...
    const knownFiles = await loadKnownScannerFiles();
    const files = walkMediaFiles(root, minFileSizeBytes);
    logger.info('library.scan_files_discovered', {
      trigger,
      root,
      fileCount: files.length,
      knownFileCount: knownFiles.size,
      forceRescan
    });
    const prepared = await prepareScanCandidates(files, root, config, counters, {
      forceRescan,
      knownFiles
    });
    const candidates = prepared.candidates;
    const currentPaths = [...prepared.skippedKnownPaths];
    ...
    counters.removedMissing = await removeMissingMovieRows();
    logger.info('library.scan_missing_rows_removed', {
      trigger,
      removedMissing: counters.removedMissing
    });
    counters.removed = await removeStaleScannerRows(currentPaths);
