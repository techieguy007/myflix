...
function ffmpegCommandOutput(args, label) {
  return new Promise((resolve) => {
    execFile(
      ...
      (error, stdout, stderr) => {
        if (error) {
          logger.warn('ffmpeg.capabilities_failed', { label, error });
...
function ffmpegEncoders() {
  return ffmpegCommandOutput(['-hide_banner', '-encoders'], 'encoders');
}

function ffmpegFilters() {
  return ffmpegCommandOutput(['-hide_banner', '-filters'], 'filters');
}

function ffmpegHwaccels() {
  return ffmpegCommandOutput(['-hide_banner', '-hwaccels'], 'hwaccels');
}
...
async function getFfmpegCapabilities(force = false) {
  ...
  const [encoders, filters, hwaccels] = await Promise.all([
    ffmpegEncoders(),
    ffmpegFilters(),
    ffmpegHwaccels()
  ]);
  const encoderText = encoders.text || '';
  const filterText = filters.text || '';
  const hwaccelText = hwaccels.text || '';
  ffmpegCapabilitiesCache = {
    ffmpegPath: commandPath('ffmpeg', 'FFMPEG_PATH'),
    h264Nvenc: /\bh264_nvenc\b/.test(encoderText),
    hevcNvenc: /\bhevc_nvenc\b/.test(encoderText),
    libx264: /\blibx264\b/.test(encoderText),
    cudaHwaccel: /(?:^|\s)cuda(?:\s|$)/i.test(hwaccelText),
    scaleCuda: /\bscale_cuda\b/.test(filterText),
    error: encoders.error || filters.error || hwaccels.error || null
  };
...
async function choosePreparedVideoEncoder(videoCodec, preference, options = {}) {
  ...
  if (options.strictDeviceCompatibility && wantsGpu && capabilities.h264Nvenc) {
    const cudaPipeline = capabilities.cudaHwaccel && capabilities.scaleCuda;
    return {
      encoder: 'h264_nvenc',
      videoAction: cudaPipeline ? 'transcode-gpu-cuda-strict-mobile' : 'transcode-gpu-strict-mobile',
      hardware: true,
      reason: cudaPipeline ? 'strict-mobile-h264-nvenc-cuda' : 'strict-mobile-h264-nvenc'
    };
  }
...
function isDeviceSafeMp4(filePath, probe) {
  ...
  const hasBFrames = Number(video && video.has_b_frames || 0);
  ...
  const videoOk = ['h264', 'avc1'].includes(videoCodec)
    && (!profile || profile.includes('baseline') || profile.includes('main'))
    && (!pixelFormat || pixelFormat === 'yuv420p')
    && (!hasBFrames || hasBFrames === 0)
...
async function preparedJobArgs(movie, options, probe, outputPath) {
  ...
  const capabilities = await getFfmpegCapabilities();
  const useCudaPipeline = videoEncoder.encoder === 'h264_nvenc'
    && capabilities.cudaHwaccel
    && capabilities.scaleCuda
    && options.disableCudaPipeline !== true;
  const nvencCpuDecodeFallback = videoEncoder.encoder === 'h264_nvenc'
    && options.disableCudaPipeline === true;
  const videoAction = nvencCpuDecodeFallback
    ? (options.strictDeviceCompatibility
      ? 'transcode-gpu-nvenc-cpu-decode-strict-mobile'
      : 'transcode-gpu-nvenc-cpu-decode')
    : videoEncoder.videoAction;
  const encoderReason = nvencCpuDecodeFallback
    ? (options.strictDeviceCompatibility
      ? 'strict-mobile-h264-nvenc-cpu-decode'
      : 'h264-nvenc-cpu-decode')
    : (useCudaPipeline ? `${videoEncoder.reason}-pipeline` : videoEncoder.reason);
  ...
  if (useCudaPipeline) {
    args.push('-hwaccel', 'cuda', '-hwaccel_output_format', 'cuda', '-extra_hw_frames', '8');
  }
  args.push(
    '-i', movie.video_path,
    '-map', '0:v:0',
  ...
  if (videoEncoder.encoder === 'copy') {
    ...
    if (strictMobile) {
      args.push(
        ...
        '-bufsize', '7000k',
        '-vf', useCudaPipeline
          ? "scale_cuda=w='min(1920,iw)':h='min(1080,ih)':format=nv12:force_original_aspect_ratio=decrease:force_divisible_by=2"
  ...
  return new Promise(async (resolve) => {
    ...
    if (typeof job.onStart === 'function') {
      try {
        ...
        logger.warn('prepared.start_callback_failed', {
          movieId: movie.id,
          variant: preparedVariantKey(options),
          error
        });
    ...
    while (!(await isBackgroundConversionPaused())) {
      ...
      await db.run(`
        ...
        SET status = 'running',
            ...
            source_path = ?,
            reason = CASE
              WHEN reason = 'resuming after service restart' THEN 'background'
              ELSE reason
            END,
      ...
      logger.info('background_conversion.job_start', {
        ...
        target: 'mobile-safe-h264-aac-mp4'
      });
      ...
      let result = await startPreparedJob({
        ...
        onStart: (details) => db.run(`
          ...
          WHERE id = ?
        `, [
          details.encoderUsed || null,
          details.videoAction || 'background',
          job.id
        ]),
      ...
      if (!result.ready
        && result.encoderUsed === 'h264_nvenc'
        && String(result.videoAction || '').includes('cuda')
        && encoderPreference(job.encoder_preference) !== 'cpu') {
        logger.warn('background_conversion.cuda_pipeline_failed_nvenc_retry', {
          jobId: job.id,
          movieId: movie.id,
          title: movie.title,
          code: result.code,
        ...
        await db.run(`
          UPDATE background_conversion_jobs
          SET reason = 'CUDA decode failed; retrying NVENC with CPU decode',
        ...
        result = await startPreparedJob({
          ...
          options: {
            ...
            disableCudaPipeline: true,
            audioStreamIndex: Number.isInteger(preferredAudioStreamIndex) ? preferredAudioStreamIndex : null,
            forceVideoTranscode: true,
            strictDeviceCompatibility: true
          },
          reason: 'background-nvenc-cpu-decode-fallback',
          onStart: (details) => db.run(`
            UPDATE background_conversion_jobs
            SET encoder_used = ?,
                reason = ?,
          ...
          `, [
            details.encoderUsed || null,
            details.videoAction || 'background-cpu-fallback',
            job.id
          ]),
          onProgress: (progress) => updateBackgroundJobProgress(job.id, progress)
        });
      ...
      if (!result.ready && result.encoderUsed === 'h264_nvenc' && encoderPreference(job.encoder_preference) !== 'cpu') {
