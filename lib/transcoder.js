...
async function getPlaybackProfile(movie, options = {}) {
  ...
  const streamMode = shouldUsePrepared
  ...
  logger.info('playback.profile_computed', {
    ...
    preparedReady: preparedStatus ? preparedStatus.ready : false,
  ...
  return {
    ...
    requiresPreparedMedia: shouldUsePrepared,
    preparedReady: preparedStatus ? preparedStatus.ready : false,
    preparedVariant: preparedStatus ? preparedStatus.variantFile : null,
    format: path.extname(movie.video_path).replace('.', '').toLowerCase(),
    selectedAudioStreamIndex,
    startSeconds,
