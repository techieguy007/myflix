...
function preparedVariantKey(options = {}) {
  const audio = Number.isInteger(options.audioStreamIndex) ? options.audioStreamIndex : 'auto';
  const compatibilitySuffix = options.forceVideoTranscode ? '-compat' : '';
...
function optionsFromPreparedVariant(fileName) {
  const match = String(fileName || '').match(/^audio-(auto|\d+)(-compat)?\.mp4$/);
  if (!match) {
    return null;
  }
...
async function getPlaybackProfile(movie, options = {}) {
  ...
  const shouldUsePrepared = forceCompatible || needsPreparedMedia(movie.video_path, probe, tracks);
  ...
  if (shouldUsePrepared && !preparedStatus.ready) {
    enqueuePreparedMedia(movie, preparedOptions, forceCompatible ? 'mobile-compatible-playback' : 'playback');
  }
...
async function setBackgroundConversionPaused(paused) {
  ...
  if (!paused) {
    processBackgroundConversionQueue();
  }
...
async function queueBackgroundConversionJobs(movies, options = {}) {
  ...
  for (const movie of movies) {
    ...
    const tracks = tracksFromProbe(probe);
    if (!force && !needsPreparedMedia(sourcePath, probe, tracks)) {
      result.skipped += 1;
      await db.run(`
        INSERT INTO background_conversion_jobs (
          movie_id, title, source_path, status, encoder_preference, reason,
          encoder_used, progress_percent, finished_at
        ) VALUES (?, ?, ?, 'skipped', ?, 'already browser compatible', 'none', 100, CURRENT_TIMESTAMP)
      `, [movie.id, movie.title, sourcePath, preference]);
      continue;
    }
...
async function startBackgroundConversionQueue(trigger = 'manual') {
  ...
  return processBackgroundConversionQueue();
}
