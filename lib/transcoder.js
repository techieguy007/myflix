...
function isDeviceSafeMp4(filePath, probe) {
  ...
  const videoOk = ['h264', 'avc1'].includes(videoCodec) && (!pixelFormat || pixelFormat === 'yuv420p');
  const audioOk = !audio || (audioCodec === 'aac' && (!channels || channels <= 2));
  return videoOk && audioOk && unsupportedStreams.length === 0;
}
