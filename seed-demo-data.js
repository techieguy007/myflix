...
console.log('🌱 MyFlix Demo Data Seeder');
console.log('==========================\n');
...
if (!fs.existsSync(dbPath)) {
  console.log('❌ Database not found. Please run reset-db.js first or start the server to initialize the database.');
  process.exit(1);
}
...
async function seedDemoData() {
  try {
    ...
    await runQuery('DELETE FROM watch_history');
    await runQuery('DELETE FROM favorites');
    await runQuery('DELETE FROM movie_categories');
    await runQuery('DELETE FROM movies');
    await runQuery('DELETE FROM categories');
    await runQuery('DELETE FROM users');
    
    // Reset auto-increment counters
    await runQuery('DELETE FROM sqlite_sequence WHERE name IN ("users", "movies", "categories", "watch_history", "favorites")');
    ...
    console.log('   User: sarah / password123');
    console.log('   User: mike / password123');
...
async function seedWatchHistory() {
  // Get user IDs
  const users = await getAllQuery('SELECT id, username FROM users WHERE is_admin = 0');
  const movies = await getAllQuery('SELECT id FROM movies LIMIT 5');
  
  if (users.length === 0 || movies.length === 0) {
    console.log('   ⚠️  Skipping watch history (no users or movies)');
    return;
  }
  
  // Create some watch history for demo users
  for (let i = 0; i < users.length; i++) {
    const user = users[i];
    const movie = movies[i % movies.length];
    
    // Random watch time (10-80% of movie duration)
    const movieData = await getQuery('SELECT duration FROM movies WHERE id = ?', [movie.id]);
    const maxWatchTime = Math.floor((movieData.duration || 120) * 0.8 * 60);
    const watchTime = Math.floor(Math.random() * maxWatchTime);
    const completed = watchTime > maxWatchTime * 0.9;
    
    await runQuery(
      'INSERT OR REPLACE INTO watch_history (user_id, movie_id, watch_time, completed, last_watched) VALUES (?, ?, ?, ?, datetime("now"))',
      [user.id, movie.id, watchTime, completed ? 1 : 0]
    );
  }
  
  console.log(`   ✅ Created watch history for ${users.length} users`);
}

async function seedFavorites() {
  // Get user IDs
  const users = await getAllQuery('SELECT id FROM users WHERE is_admin = 0');
  const movies = await getAllQuery('SELECT id FROM movies');
  
  if (users.length === 0 || movies.length === 0) {
    console.log('   ⚠️  Skipping favorites (no users or movies)');
    return;
  }
  
  // Each user favorites 2-3 random movies
  for (const user of users) {
    const favoriteCount = 2 + Math.floor(Math.random() * 2); // 2-3 favorites
  ...
  console.log(`   ✅ Created favorites for ${users.length} users`);
}
